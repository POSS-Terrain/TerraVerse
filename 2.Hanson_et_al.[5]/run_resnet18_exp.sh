#!/bin/bash
# 脚本名称：run_resnet18_exp.sh
# 功能：依次运行ResNet18_final.py从exp1到exp15，指定GPU 1，前一个失败则终止，最后跑exp21

# 可配置参数（按需修改）
SCRIPT_NAME="ResNet18_final.py"  # 要执行的Python脚本名
GPU_ID=1                         # 指定使用的GPU编号
START_EXP=1                      # 起始实验编号
END_EXP=4                        # 结束实验编号
...
# 所有实验完成提示
echo "🎉 所有实验（exp1到exp${END_EXP}，已跳过exp7，以及exp21）均执行完成！"
exit 0