...
# ==============================================================================
parser = argparse.ArgumentParser(description="多卡并发训练 ResNet18 - Few-Shot Adaptation")
...
CONFIG = {
    ...
    "num_workers": 8,
    "img_size": 224,
    "save_dir": "./resnet18_checkpoints/benchmark5"
}
...
def main(args):
    ...
    best_base_model_path = os.path.join(exp_save_dir, "resnet18_base_best.pth")
    best_fs_model_path = os.path.join(exp_save_dir, "resnet18_fewshot_best.pth")

    print(f"\n=======================================================")
    print(f"🚀 启动 ResNet18 Few-Shot 实验: {args.exp_name} | GPU {args.gpu}")
    print(f"=======================================================\n")
    ...
    optimizer_base = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=CONFIG["lr"])
    best_base_acc = 0.0
    patience = 5 
    counter = 0 
    ...
    # 降低学习率进行 fine-tune
    optimizer_fs = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=CONFIG["lr"] * 0.1)
