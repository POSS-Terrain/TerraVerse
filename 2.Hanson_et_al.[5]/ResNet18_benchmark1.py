...
CONFIG = {
    ...
    "num_workers": 8,
    "img_size": 224,
    "save_dir": "./resnet18_checkpoints/benchmark1"
}
...
def main(args):
    ...
    # 修复了原来写成 valid_dirs 的 bug，现在统一读取 test_dirs
    ...
    final_acc, all_labels, all_preds = evaluate_resnet(model, test_loader, CONFIG["device"])
    ...
    for subset_name, dir_list in grouped_test_dirs.items():
        ...
        # 评估该子集
        sub_acc, sub_targets, sub_preds = evaluate_resnet(model, subset_loader, CONFIG["device"])
