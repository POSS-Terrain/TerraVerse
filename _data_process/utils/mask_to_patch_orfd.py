...
def color_to_label_ids(mask_rgb: np.ndarray) -> np.ndarray:
    """Convert RGB color mask (H,W,3) to a single integer id per color for histogram.
    We don't need semantic mapping here, only majority ratio (>95%).
    id = (R<<16) + (G<<8) + B
    """
    if mask_rgb.ndim == 2:
        # already single channel, return as-is
        return mask_rgb
...
def iter_patches(img: np.ndarray, patch: int = 224, stride: Optional[int] = None):
    # Default stride to half of patch when not provided
...
def process_split(
    ...
    # Fixed mapping from packed color IDs to semantic names
    # id = (R<<16) + (G<<8) + B
    color_label_map = {0: "non-traversable", 128: "sky", 255: "traversable"}
    ...
    # for naming: img_000001_0001.png etc.
    for i, it in enumerate(items, start=1):
        ...
        # ensure mask is at least 2D, handle RGB fillcolor or single-channel
        ...
        for r, c in iter_patches(img, patch=patch_size, stride=stride):
            ...
            maj_color, ratio = majority_ratio(mask_ids_patch)
            if ratio < min_majority:
                continue

            patch_idx += 1
            base = Path(img_name).stem  # e.g., img_000001
            out_img_name = f"{base}_{patch_idx:04d}.png"
            save_png(out_dir / out_img_name, img_patch)

            # Map packed color ID to semantic label; skip unknown colors
            label_name = color_label_map.get(maj_color)
            ...
            label_items.append(
                {
                    "name": out_img_name,
                    "label": label_name,
                    # Store patch corners in source-image coordinates (origin at top-left)
    ...
    # write local label json (image name -> label id)
    ...
    return len(results), len(set(color_label_map.values()))


def main(argv: Sequence[str] | None = None):
    parser = argparse.ArgumentParser(description="Mask to patch processor (224x224, stride=half patch by default, >95% single label)")
    parser.add_argument(
        "--processed-root",
        type=Path,
    ...
    for split in split_names:
        ...
        print(
            f"  -> accepted patches: {n_patches}, labels: {n_labels} (output: {split_root / 'local_image'})"
        )
