...
# Default settings follow the prior ORFD patch extractor: 224px saved size, decision patch = 224, stride=112 for 224 patch, 95% majority.
DEFAULT_PATCH_SIZE = 224
DEFAULT_TARGET_SIZE = 224
DEFAULT_DECISION_PATCH_SIZES = [DEFAULT_PATCH_SIZE]
DEFAULT_STRIDE = DEFAULT_PATCH_SIZE // 2
DEFAULT_THRESHOLD = 0.95


def ensure_clean_dir(path: Path, reset: bool) -> None:
	"""Create target directory and optionally clear existing content."""
	...
	for p in path.iterdir():
		if p.is_file() or p.is_symlink():
			...
			# This directory should normally be flat, but be safe.
			for sub in p.rglob("*"):
...
def load_png(path: Path) -> np.ndarray:
	"""Load image/mask as numpy array (uint8)."""
...
def load_label_mapping(mapping_path: Path) -> Tuple[Dict[int, str], Dict[int, int]]:
	"""
	Load goose_label_mapping.csv and return:
	- id_to_name: label_key (int) -> class_name (str)
	- color_to_id: packed RGB int -> label_key (for color masks)
	"""
...
def mask_to_label_ids(mask: np.ndarray, color_to_id: Dict[int, int]) -> np.ndarray:
	"""
	Convert mask to label-id array.
	- If mask is RGB, map each color to label id (unknown -> 0).
	- If mask is single-channel, assume it already stores label ids.
	"""
	if mask.ndim == 2:
	...
	h, w, c = mask.shape
	if c >= 3:
		# Pack RGB into int for fast lookup.
...
def process_split(
	...
	for idx, entry in enumerate(items, start=1):
		...
		h, w = label_ids.shape
		base = Path(entry["img"]).stem  # e.g., img_000001
		patch_idx = 0
		for patch_size in decision_patch_sizes:
			for y, x in iter_patch_origins(h, w, patch_size, stride):
				...
				# Upsample to target size for downstream model consumption.
			...
			# limit-images applies to number of source images, not patches
...
def main(argv: Sequence[str] | None = None):
	parser = argparse.ArgumentParser(
		description="Extract GOOSE patches with label names (224 decision patches, upsampled to 224, >95% majority by default)",
	)
	parser.add_argument(
		"--processed-root",
		type=Path,
	...
	parser.add_argument(
		...
		help="Fallback mapping root; defaults to data/GOOSE and prefers data/GOOSE/goose_label_mapping.csv",
	)
	parser.add_argument(
		"--split",
		type=str,
	...
	parser.add_argument(
		...
		help="Decision patch sizes to evaluate (e.g., 224); default is 224",
	)
	parser.add_argument(
		"--stride",
		type=int,
		default=None,
		help="Stride for sliding window; default=half of the current decision patch",
	)
