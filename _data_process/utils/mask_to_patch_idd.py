...
def load_label_color_map(label_map_path: Path) -> Dict[int, str]:
	...
	for entry in items:
		...
		if not isinstance(name, str) or not isinstance(color, list) or len(color) != 3:
			continue
		r, g, b = [int(c) for c in color]
		packed = (r << 16) + (g << 8) + b
		color_map[packed] = name
	return color_map
...
def process_split(
	...
	reset: bool = True,
	color_label_map: Optional[Dict[int, str]] = None,
) -> Tuple[int, int]:
	global_dir = split_root / "global_image"
	out_dir = split_root / "local_image"
...
def main(argv: Sequence[str] | None = None) -> None:
	parser = argparse.ArgumentParser(description="IDD mask-to-patch processor (224x224, default stride=half patch, >95% single label)")
	parser.add_argument(
		"--processed-root",
		type=Path,
	...
	label_map_path = args.processed_root / "label_map.json"
	color_label_map = load_label_color_map(label_map_path)
