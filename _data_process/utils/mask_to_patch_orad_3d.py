...
def label_for_id(majority_id: int, mode: str) -> str:
	if mode == "color":
		return f"color_{majority_id:06x}"
	label_map = {
		0: "non-road",
		128: "sky",
		255: "road",
	}
	return label_map.get(majority_id, f"id_{majority_id}")
...
def process_split(
	...
	for i, it in enumerate(items, start=1):
		...
		for r, c in iter_patches(img, patch=patch_size, stride=stride):
			...
			label_name = label_for_id(int(majority_id), mode)
