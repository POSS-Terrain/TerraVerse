...
CONFIG = {
    ...
    "num_workers": 8,
    "img_size": 224,
    "save_dir": "./efficientnet_checkpoints/benchmark1",
...
def evaluate_efficientnet(model, loader, device):
    """
    专门为包含 CenterLoss 的 EfficientNet 抽取的评估函数
    注意前向传播接收的是两个返回值，评估只需要 logits
    """
    model.eval()
    all_preds = []
    all_labels = []
    with torch.no_grad():
        for images, labels in tqdm(loader, desc="Evaluating", leave=False):
            ...
            # EfficientNet 这套代码返回 (embedding, logits)
...
def main(args):
    ...
    final_acc, all_labels, all_preds = evaluate_efficientnet(model, test_loader, CONFIG["device"])
    ...
    for subset_name, dir_list in grouped_test_dirs.items():
        ...
        # 评估该子集
        sub_acc, sub_targets, sub_preds = evaluate_efficientnet(model, subset_loader, CONFIG["device"])
