...
# ==============================================================================
parser = argparse.ArgumentParser(description="EfficientNet 纯测试脚本 (CenterLoss 模型)")
...
parser.add_argument("--ckpt_path", type=str, 
                    default="/8TBHDD3/tht/TerraX/8Zhao_et_al./efficientnet_checkpoints/benchmark1/exp2/efficientnet_traversable_best.pth", 
                    help="训练好的 EfficientNet 模型权重路径")
...
# ==========================================
# PART 4: EfficientNet 模型 (移除 CenterLoss 类定义和预训练权重)
# ==========================================
...
def main(args):
    ...
    print(f"\n=======================================================")
    print(f"🚀 启动 EfficientNet 测试: {args.exp_name} | 使用设备: GPU {args.gpu}")
    ...
    if os.path.exists(args.ckpt_path):
        # 即使训练时有 center_loss 权重被保存到了其它地方，只要加载 model.state_dict 即可恢复分类器能力
    ...
    with torch.no_grad():
        for images, labels in tqdm(test_loader, desc="Final Testing"):
            ...
            # 测试阶段忽略 embeddings，只取 logits 用于分类预测
    ...
    csv_filename = f"{args.exp_name}_efficientnet_inference_results.csv"
