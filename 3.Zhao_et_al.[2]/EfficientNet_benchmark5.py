...
# ==============================================================================
parser = argparse.ArgumentParser(description="多卡并发训练 EfficientNet (Center Loss) - Few-Shot Adaptation")
...
CONFIG = {
    ...
    "batch_size": 256,
    "base_epochs": 80, 
    "fewshot_epochs": 50, # Few-shot 微调 Epochs
    "device": "cuda" if torch.cuda.is_available() else "cpu",
    "num_workers": 8,
    "img_size": 224,
    "save_dir": "./efficientnet_checkpoints/benchmark5", 
...
# ==========================================
# PART 4: EfficientNet 模型与 CenterLoss
# ==========================================
...
# ==========================================
# PART 5: 主程序 (Base Training + 1% Fine-Tuning)
# ==========================================
def main(args):
    ...
    best_base_model_path = os.path.join(exp_save_dir, "efficientnet_base_best.pth")
    best_fs_model_path = os.path.join(exp_save_dir, "efficientnet_fewshot_best.pth")

    print(f"\n=======================================================")
    print(f"🚀 启动 EfficientNet Few-Shot 实验: {args.exp_name} | GPU: {args.gpu}")
    print(f"=======================================================\n")
    ...
    for epoch in range(CONFIG["base_epochs"]):
        model.train()
        for images, labels in tqdm(source_train_loader, desc=f"Base Train Ep {epoch+1}"):
        ...
        with torch.no_grad():
            for images, labels in source_valid_loader:
                images, labels = images.to(CONFIG["device"]), labels.to(CONFIG["device"])
        ...
        if val_acc > best_base_acc + 0.001: 
            ...
            # 注意：如果打算后续推理还需要 center loss 参数，理论上也该存下来，
            # 但针对分类准确率评估，存 model 的 state_dict 即可
    ...
    # 微调阶段：降低学习率
    optimizer_model_fs = optim.Adam(model.parameters(), lr=CONFIG['lr_backbone'] * 0.1)
    optimizer_center_fs = optim.Adam(center_loss.parameters(), lr=CONFIG['lr_center'] * 0.1)
    
    for epoch in range(CONFIG["fewshot_epochs"]):
        model.train()
        for images, labels in target_fewshot_loader:
            images, labels = images.to(CONFIG["device"]), labels.to(CONFIG["device"])
            ...
            optimizer_model_fs.zero_grad()
            optimizer_center_fs.zero_grad()
            
            loss.backward()
            
            optimizer_model_fs.step()
            for param in center_loss.parameters():
                param.grad.data *= (1. / CONFIG['lambda_center']) 
            optimizer_center_fs.step()
            
    torch.save(model.state_dict(), best_fs_model_path)

    # =======================================================
    # STAGE 3: Final Testing (在 Target Test 数据上盲测)
    # =======================================================
    ...
    all_preds, all_labels = [], []
    with torch.no_grad():
        for images, labels in tqdm(target_test_loader, desc="Final Testing"):
            images, labels = images.to(CONFIG["device"]), labels.to(CONFIG["device"])
            _, logits = model(images)
            preds = torch.argmax(logits, dim=1)
            all_preds.extend(preds.cpu().numpy())
