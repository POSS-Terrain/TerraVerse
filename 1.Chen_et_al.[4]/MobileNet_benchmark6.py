...
# ==========================================
# PART 1: 全局标签定义 & 配置参数 [修改点：改为可通行性二分类]
# ==========================================
...
CONFIG = {
    ...
    "num_workers": 8,
    "img_size": 224,
    "save_dir": "./mobilenet_checkpoints/benchmark6" 
}

# ==========================================
# PART 2: 数据集类 [修改点：废除文件夹模式，读取 traversability 字段]
# ==========================================
    ...
    def __init__(self, dir_list, json_name, transform=None, is_train=False):
        ...
        for folder_path in dir_list:
            ...
            if os.path.exists(json_path):
                ...
                # [修改点]：如果没有 json 文件，直接跳过并警告，废弃原本的 _load_folder_mode
    ...
    def _load_json_mode(self, folder_path, json_path):
        try:
            ...
            for item in items:
                ...
                # [修改点]：提取 traversability 而不是 label
...
def main(args):
    ...
    # [修改点]：更换了模型保存的文件名，方便和之前的多分类任务作区分
    best_model_path = os.path.join(exp_save_dir, "mobilenet_traversable_best.pth")
