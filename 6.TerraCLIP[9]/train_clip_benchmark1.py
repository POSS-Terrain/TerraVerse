...
CONFIG = {
    ...
    "save_dir": "./clip_checkpoints/benchmark1",
    "lora_rank": 8,
    "lora_alpha": 16,
    "lora_dropout": 0.1,
    "wandb_project": "TerraVerse-TerraCLIP-Benchmark1",
}
...
# ==============================================================================
# 在 main 函数外新增一个实用函数，用于自动对 test_dirs 进行分组
# ==============================================================================
def group_test_dirs(test_dirs):
    """
    根据路径自动将测试集归类。
    例如将含 "RUGD" 的 test 和 valid 路径都归为 "RUGD" 这个子集。
    """
    from collections import defaultdict
    grouped = defaultdict(list)
    for path in test_dirs:
        # 通过字符串截取，自动提取数据集主名称
        # 例如 "/.../TerraData/RUGD/processed..." -> 提取出 "RUGD"
        if "TerraData/" in path:
            dataset_name = path.split("TerraData/")[1].split("/")[0]
            # 特殊处理 GOOSE，因为它下面还有 goose 和 gooseEx
            if dataset_name == "GOOSE":
                dataset_name = "GOOSE_" + path.split("GOOSE/")[1].split("/")[0]
        else:
            # 如果路径不规范，用最后几级目录作为名字的 fallback
...
# ==============================================================================
    ...
    for subset_name, dir_list in grouped_test_dirs.items():
        ...
        # 针对当前子集实例化 Dataset 和 Loader
        subset_dataset = TerraHybridCLIPDataset(dir_list, transform=valid_transform_safe, is_train=False)
        ...
        # 评估该子集
        sub_acc, sub_targets, sub_preds = evaluate(model, subset_loader, device, ALL_TERRAIN_CLASSES)
    ...
    # 按准确率降序排列（保留第一行 Overall 不动，剩下的排序以便直观看出在哪泛化得好）
    ...
    print("\n=======================================================")
    print(f"🎯 所有测试已完成！")
    print(f"📄 总体详细报告: {overall_csv_path}")
    print(f"📊 各子集准确率汇总 (最适合画图/表格): {summary_csv_path}")
