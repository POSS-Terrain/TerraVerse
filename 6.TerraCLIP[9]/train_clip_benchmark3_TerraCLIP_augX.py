...
class SpecificAugTransform:
    def __init__(self, augmenter):
        """
        不再使用随机概率，而是明确传入一个特定的增强器，100% 触发。
        """
        self.aug = augmenter
...
CONFIG = {
    ...
    "save_dir": "./clip_checkpoints/benchmark3_augX",
    "lora_rank": 8,
    "lora_alpha": 16,
    "lora_dropout": 0.1,
    "wandb_project": "TerraVerse-TerraCLIP-Benchmark3_augX",
}
    ...
    def _load_test_labels(self, folder_path, json_path):
        ...
        try:
            ...
            for item in items:
                ...
                # 先检查 img_name 和 label_str 是否有效，再转小写
...
def main(args):
    ...
    train_transform_clean = transforms.Compose(base_ops)

    # 1. 定义 9 种独立的特效 Transform
    def get_specific_transform(augmenter):
        return transforms.Compose([
            transforms.Resize((CONFIG["img_size"], CONFIG["img_size"])), 
            SpecificAugTransform(augmenter), # 插入 100% 触发的特定特效
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize(clip_mean, clip_std)
        ])

    transform_blur = get_specific_transform(iaa.imgcorruptlike.MotionBlur(severity=(1, 2)))
    transform_fog = get_specific_transform(iaa.imgcorruptlike.Fog(severity=1))
    transform_snow = get_specific_transform(iaa.imgcorruptlike.Snow(severity=2))
    transform_rain = get_specific_transform(iaa.Rain(drop_size=(0.10, 0.15), speed=(0.1, 0.2)))
    transform_flakes = get_specific_transform(iaa.Snowflakes(flake_size=(0.1, 0.4), speed=(0.01, 0.03)))
    transform_contrast = get_specific_transform(iaa.imgaug.augmenters.contrast.LinearContrast((0.5, 2.0), per_channel=0.5))
    transform_bright = get_specific_transform(iaa.imgcorruptlike.Brightness(severity=(1, 2)))
    transform_saturate = get_specific_transform(iaa.imgcorruptlike.Saturate(severity=(1, 3)))
    transform_dark = get_specific_transform(iaa.Multiply((0.1, 0.5)))
    ...
    # 分别挂载 10 种不同的 Transform 管道
    d_clean = DatasetWrapper(raw_train_set, transform=train_transform_clean)
    d_blur = DatasetWrapper(raw_train_set, transform=transform_blur)
    d_fog = DatasetWrapper(raw_train_set, transform=transform_fog)
    d_snow = DatasetWrapper(raw_train_set, transform=transform_snow)
    d_rain = DatasetWrapper(raw_train_set, transform=transform_rain)
    d_flakes = DatasetWrapper(raw_train_set, transform=transform_flakes)
    d_contrast = DatasetWrapper(raw_train_set, transform=transform_contrast)
    d_bright = DatasetWrapper(raw_train_set, transform=transform_bright)
    d_saturate = DatasetWrapper(raw_train_set, transform=transform_saturate)
    d_dark = DatasetWrapper(raw_train_set, transform=transform_dark)
    
    # 🌟 核心魔法：使用 ConcatDataset 拼装成 10 倍超大数据集
    from torch.utils.data import ConcatDataset
    real_train_dataset = ConcatDataset([
        d_clean, d_blur, d_fog, d_snow, d_rain, 
        d_flakes, d_contrast, d_bright, d_saturate, d_dark
    ])
    ...
    # 🌟 新增：将测试结果转换为 DataFrame 并保存为 CSV
