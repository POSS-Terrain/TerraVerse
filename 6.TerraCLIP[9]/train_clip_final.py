...
CONFIG = {
    ...
    "save_dir": "./clip_checkpoints/benchmark4",
    "lora_rank": 8,
    "lora_alpha": 16,
    "lora_dropout": 0.1,
    "wandb_project": "TerraVerse-TerraCLIP-Benchmark4",
}
...
#     model.eval()
#     print("Computing multi-granularity text prototypes for Inference...")
    
#     # ==========================================
#     # 1. 构建粗粒度文本 (Coarse-grained prompts)
#     # ==========================================
...
#     # ==========================================
#     # 2. 根据标注规则构建细粒度属性池 (M=21 个属性)
#     # 这就是论文 Section IV.D 提到的专家定义的属性集 A_i
#     # ==========================================
#     fine_attributes = {
...
#         # 提取细粒度特征 z_{i,j}^{T^f} [K, M, D]
#         K = len(all_labels)
#         fine_features_list = []
        
#         for label in all_labels:
#             class_fine_prompts = []
#             # 遍历所有 21 个可能的属性，生成 M=21 句细粒度 prompt
#             for attr_key, attr_vals in fine_attributes.items():
#                 for val in attr_vals:
#                     prompt = f"The terrain material is {label}, {attr_key} is {val}."
#                     class_fine_prompts.append(prompt)
            
#             fine_tokens = clip.tokenize(class_fine_prompts, truncate=True).to(device)
#             features = model.encode_text(fine_tokens) # [M, D]
#             features = F.normalize(features, dim=-1)
#             fine_features_list.append(features)
            
#         fine_features = torch.stack(fine_features_list, dim=0) # [K, M, D]
        
#     all_preds = []
#     all_targets = []
    
#     # 论文 Section V.A.2 中的超参数: 显著性增强因子 \beta = 0.3
#     beta = 0.3 
...
#         for images, _, _, labels in tqdm(loader, desc="Validating (Multi-Granularity)"):
#             images = images.to(device)
...
#             # ----------------------------------------------------
#             # 论文 公式 (5): 计算粗粒度相似度 s^c (s_c)
#             # ----------------------------------------------------
#             s_c = logit_scale * (image_features @ coarse_features.t()) # [B, K]
            
#             # 计算图像与所有类别、所有细粒度属性的相似度矩阵
#             # shape: [Batch, K_classes, M_attributes]
#             sim_matrix = logit_scale * torch.einsum('bd,kmd->bkm', image_features, fine_features)
            
#             # ----------------------------------------------------
#             # 论文 公式 (6): 计算细粒度相似度 s^f
#             # s^f_i = mean_j(z^I * z^{T^f}) + \beta * max_j(z^I * z^{T^f})
#             # ----------------------------------------------------
#             mean_sim = sim_matrix.mean(dim=-1) # [B, K]
#             max_sim = sim_matrix.max(dim=-1).values # [B, K]
#             s_f = mean_sim + beta * max_sim # [B, K]
            
#             # ----------------------------------------------------
#             # 计算置信度权重 \lambda = 1 - \mathcal{H}(S^c)
#             # ----------------------------------------------------
#             # 1. 将粗粒度相似度转为概率分布 (Softmax)
#             p_c = F.softmax(s_c, dim=-1) # [B, K]
            
#             # 2. 计算信息熵 \mathcal{H}(S^c)
#             # 除以 log(K) 是为了将熵归一化到 [0, 1] 区间，防止 \lambda 变成负数
#             entropy = -torch.sum(p_c * torch.log(p_c + 1e-8), dim=-1) / torch.log(torch.tensor(K, dtype=torch.float32).to(device))
            
#             # 3. 计算 \lambda
#             lambda_weight = (1.0 - entropy).unsqueeze(-1) # [B, 1]
            
#             # ----------------------------------------------------
#             # 论文 公式 (7): 最终融合预测 \hat{l^c}
#             # ----------------------------------------------------
#             final_scores = lambda_weight * s_c + (1.0 - lambda_weight) * s_f # [B, K]
