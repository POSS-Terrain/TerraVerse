...
CONFIG = {
    ...
    "num_workers": 8,
    "img_size": 224,
    "save_dir": "./dinov2_checkpoints/benchmark1",
...
def evaluate_dinov2(model, loader, device):
    """
    专门为 DINOv2 抽取的评估函数
    """
    model.eval()
...
def main(args):
    ...
    final_acc, all_labels, all_preds = evaluate_dinov2(model, test_loader, CONFIG["device"])
    ...
    for subset_name, dir_list in grouped_test_dirs.items():
        ...
        # 评估该子集
        sub_acc, sub_targets, sub_preds = evaluate_dinov2(model, subset_loader, CONFIG["device"])
