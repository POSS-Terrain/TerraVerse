...
# ==============================================================================
parser = argparse.ArgumentParser(description="多卡并发训练 DINOv2 Baseline - Few-Shot Adaptation")
...
CONFIG = {
    ...
    "num_workers": 8,
    "img_size": 224,
    "save_dir": "./dinov2_checkpoints/benchmark5",
...
def main(args):
    ...
    best_base_model_path = os.path.join(exp_save_dir, "dinov2_base_best.pth")
    best_fs_model_path = os.path.join(exp_save_dir, "dinov2_fewshot_best.pth")

    print(f"\n=======================================================")
    print(f"🚀 启动 DINOv2 Few-Shot 实验: {args.exp_name} | GPU: {args.gpu}")
    print(f"=======================================================\n")
    ...
    # 只优化分类头
    optimizer_base = optim.AdamW(model.classifier.parameters(), lr=CONFIG["lr"])
    ...
    for epoch in range(CONFIG["base_epochs"]):
        model.train()
        model.backbone.eval() # 强制 Backbone 处于 eval
    ...
    # 微调阶段：由于 DINOv2 之前只练了头，所以微调时依然只微调分类头，降低一点学习率
    optimizer_fs = optim.AdamW(model.classifier.parameters(), lr=CONFIG["lr"] * 0.1)
    
    for epoch in range(CONFIG["fewshot_epochs"]):
        model.train()
        model.backbone.eval() # 依然保持 Backbone 冻结
